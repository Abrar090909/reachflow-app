...
import { Plus, X, Mail, Shield, Zap } from 'lucide-react';

export default function Accounts() {
  const [accounts, setAccounts] = useState([]);
  const [showZoho, setShowZoho] = useState(false);
  const [zohoForm, setZohoForm] = useState({ email: '', smtp_pass: '', smtp_host: 'smtp.zoho.com', smtp_port: 465 });
  ...
  const handleZoho = async (e) => {
    ...
    try {
      ...
      setShowZoho(false);
      setZohoForm({ email: '', smtp_pass: '', smtp_host: 'smtp.zoho.com', smtp_port: 465 });
      fetchAccounts();
      ...
      {!accounts.length && (
        <div className="empty-state" style={{ marginTop: 40 }}>
          ...
          <p style={{ fontSize: 13, color: '#94a3b8', maxWidth: 320, lineHeight: 1.6, marginBottom: 20 }}>
            Connect a Gmail or Zoho email account to start sending outreach campaigns and warming up your sender reputation.
          </p>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={handleGmail} className="btn-secondary">
        ...
        <div className="modal-overlay" onClick={() => setShowZoho(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', fontFamily: 'var(--font-display)' }}>Add Zoho Account</h2>
                <p style={{ fontSize: 13, color: '#94a3b8', marginTop: 4 }}>Connect via SMTP with an app password</p>
              </div>
            ...
            <form onSubmit={handleZoho} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div>
                ...
                <input type="email" required value={zohoForm.email} onChange={(e) => setZohoForm({ ...zohoForm, email: e.target.value })}
                  className="input" placeholder="you@zoho.com" />
              </div>
              <div>
                <label className="section-label" style={{ display: 'block', marginBottom: 8 }}>App Password</label>
                <input type="password" required value={zohoForm.smtp_pass} onChange={(e) => setZohoForm({ ...zohoForm, smtp_pass: e.target.value })}
                  className="input" placeholder="••••••••••••" />
                <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <Shield size={11} /> Use a Zoho App Password, not your login password
                </p>
              </div>
              <button type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '14px 0', marginTop: 4, fontSize: 14 }}>
                Connect Account
              </button>
            </form>
          </div>
