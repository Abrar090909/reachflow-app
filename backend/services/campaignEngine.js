...
export async function runCampaigns() {
  ...
  for (const campaign of campaigns) {
    const pendingLeads = db.prepare("SELECT * FROM leads WHERE campaign_id = ? AND status = 'pending' LIMIT ?").all(campaign.id, campaign.daily_limit);
    const sentLeads = db.prepare("SELECT * FROM leads WHERE campaign_id = ? AND status = 'sent' AND replied_at IS NULL").all(campaign.id);
    let accountIndex = 0;
    ...
    for (const lead of pendingLeads) {
      ...
      try {
        ...
        db.prepare("UPDATE leads SET status = 'sent', sent_at = datetime('now'), assigned_account_id = ? WHERE id = ?").run(account.id, lead.id);
    ...
    for (const lead of sentLeads) {
      ...
      if (campaign.follow_up_1_body && !lead.follow_up_1_sent_at && daysSinceSent >= campaign.follow_up_1_delay_days) {
        try {
          ...
          db.prepare("UPDATE leads SET follow_up_1_sent_at = datetime('now') WHERE id = ?").run(lead.id);
      ...
      if (campaign.follow_up_2_body && lead.follow_up_1_sent_at && !lead.follow_up_2_sent_at && daysSinceF1 >= campaign.follow_up_2_delay_days) {
        try {
          ...
          db.prepare("UPDATE leads SET follow_up_2_sent_at = datetime('now') WHERE id = ?").run(lead.id);
        } catch (err) { console.error(`[Campaign] Follow-up 2 error:`, err.message); }
      }
    }

    const remaining = db.prepare("SELECT COUNT(*) as count FROM leads WHERE campaign_id = ? AND status = 'pending'").get(campaign.id);
